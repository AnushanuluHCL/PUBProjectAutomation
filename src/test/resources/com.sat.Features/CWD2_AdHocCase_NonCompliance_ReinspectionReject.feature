@CWD2AdHocCaseForReInspectionReject
Feature: Testing CWD2- AdHoc Case for Non-Compliance.
  ...
  Scenario: AdHoc Case for Non-Compliance ReInspection Reject flow
	...
	Then verify Rectification Documents Verified is marked as "---" and Rectification Required is marked as "Yes"
	And go to "Work Orders" tab
	And open "ECM Setup and Drainage Work Inspection" work order with filter Incident Type
	...
	And click on save button
	And click on Save&Close button
	Then go to "All Activities" and verify the email for "Resubmission of Rectification Report for"
	And logout from the application
	...
	And go to "Work Orders" tab
	And open "Scheduled" WO and fill the Booking details and select "In Progress" status
	And open "In Progress" WO fill the checklist for ECM Setup and Drainage Work inspection "ECM Setup and Drainage Work Inspection" for "Compliance" and verify WO status as "Completed"
	And go to "Inspection Case Information" tab
